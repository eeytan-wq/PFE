...
# 1. INITIALISATION & ÉQUILIBRE EXACT (CAS 2)

def get_equilibrium_state_case2(x_grid):
    """Retourne l'état d'équilibre exact pour le CAS 2. z = 0 (Fond plat), u = 0.
    Invariant : theta * h^2 = K (cste)"""
    # Paramètres
    H0 = 1.0
    A_h = 0.2
    K = 1.0
    ...
    # 1. Topographie plate
    z = z0 * np.ones_like(x_grid)
    
    # 2. Hauteur d'eau perturbée (profil initial)
    h_eq = H0 + A_h * np.sin(2.0 * np.pi * x_grid)
    
    # 3. Vitesse nulle
    q_eq = np.zeros_like(x_grid)
    
    # 4. Densité calculée pour équilibre hydrostatique
    # Invariant : theta * h^2 = K  =>  theta = K / h^2
    # m = h * theta = K / h
    theta_eq = K / np.maximum(h_eq**2, 1e-12)
    m_eq = h_eq * theta_eq # = K / h_eq
    
    return h_eq, q_eq, m_eq, z, K

# On stocke l'équilibre globalement
h_eq_glob, q_eq_glob, m_eq_glob, z_glob, K_glob = get_equilibrium_state_case2(x)
...
def reconstruction_weno5(u):
    ...
    # Reconstruction à l'interface i+1/2 (Droite)
...
def integrate_source_simpson(hL, mL, hR, mR, g=9.81):
    """ Intégration du terme source -g*h*theta pour Simpson. Dans le Cas 2, z=cste (fond plat), donc dz/dx = 0."""
    eps = 1e-12
...
def solveur_deviation(du_h, du_q, du_m):
    """Solveur Well-Balanced par perturbation. Résout dU/dt + div(F_tot - F_eq) = S_tot - S_eq"""
    eps = 1e-12
    ...
    # Attention : ici on passe bien les états d'équilibre gauche et DROITE (m_eqR)
...
def compute_rhs_deviation(du_h, du_q, du_m):
    ...
    # Pour le Cas 2, z est constant => dz = 0 => S_vol = 0
...
def simulate():
    ...
    inv_hist = [(theta_tot * (h_tot**2)).copy()]
    th_hist = [theta_tot.copy()]
    
    t = 0.0
    next_save = 0.05
    
    print("Début simulation Cas 2 (WB Perturbation)...")
    ...
    while t < Tfinal:
        ...
        if t >= next_save:
            ...
            # Invariant Cas 2 : theta * h^2
            curr_inv = theta_tot * (h_tot**2)
    ...
    err = np.max(np.abs(inv_matrix - K_glob))
    ...
    print("-" * 50)
    print(f"ERREUR MAX Invariant : {err:.5e}")
    ...
    return times, H_hist, th_hist, inv_hist, z_glob, K_glob

# 6. ANIMATION
def create_gif(times, H_hist, th_hist, inv_hist, z, K_val, x_grid):
    ...
    # Recalcul théorique de theta initial pour le graphe 3
    # au temps t=0, on est à l'équilibre exact
    # theta_ref = K / h^2
    # on reconstruit h initial : H_hist[0] - z
    h_init = H_arr[0] - z
    theta_ref = K_val / (h_init**2)

    fig, axs = plt.subplots(3, 1, figsize=(10, 12))
    fig.suptitle("Modèle de Ripa 1D — Well Balanced (Perturbation)\n"
                 r"Cas 2: Fond plat, $\theta h^2 = K$", fontsize=14)
    ...
    # Graph 2 : Invariant
    # K_val est la référence exacte
    ax2.plot(x_grid, K_val * np.ones_like(x_grid), "r--", label=f"Exact (K={K_val})")
    lineInv, = ax2.plot(x_grid, inv_arr[0], "g-", label="Calculé")
    
    # Zoom fort pour montrer la précision WB
    margin = 1e-10
    ax2.set_ylim(1-1e-8,1+1e-8) 
    ax2.grid(True, alpha=0.3)
    ax2.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
    ax2.set_ylabel(r"Invariant $\theta h^2$")
    ax2.legend(loc="upper right", fontsize=9)
    
    # Graph 3 : Température
    ax3.plot(x_grid, theta_ref, "r--", label=r"Ref ($\theta_{t=0}$)")
    lineTh, = ax3.plot(x_grid, th_arr[0], "k-", label="Calculé")
    ax3.set_ylabel(r"Température $\theta$")
    ax3.grid(True, alpha=0.3)
    
    th_min, th_max = np.min(theta_ref), np.max(theta_ref)
    pad = 0.1 * (th_max - th_min) if (th_max - th_min) > 1e-5 else 0.1
    ...
    gif_path = os.path.join(output_dir, "Cas 2 wb pertu.gif")
    ...
    png_path = os.path.join(output_dir, "Cas 2 wb pertu.png")
    fig.savefig(png_path, dpi=200)
    print(f"Succès ! Image sauvegardée : {png_path}")

if __name__ == "__main__":
    times, H_hist, th_hist, inv_hist, z, K_val = simulate()
    create_gif(times, H_hist, th_hist, inv_hist, z, K_val, x)